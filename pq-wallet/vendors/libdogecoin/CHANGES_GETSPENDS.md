...
4. **Txid mismatch vs `/getTransactions`** — `/getSpends` used `utils_reverse_hex` on `tx_hash_cache` while `spend_txid` from `dogecoin_wallet_sent_payment_hints_for_prevout` uses `dogecoin_tx_hash` hex **without** that reversal, so the same spend appeared as two different 64-char strings (e.g. `864476…` vs `a9d6026a…`). `/getSpends` now prints `txid` with `utils_bin_to_hex` only, matching `spend_txid`.
...
| `src/wallet.c` | Payment hint scan skips 0-value + OP_RETURN |
